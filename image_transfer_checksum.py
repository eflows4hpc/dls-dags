        ...
        with ssh_hook.get_conn() as ssh_client:
            ...
            with requests.get(url, stream=True, verify=False, timeout=None) as r:
                with sftp_client.open(remote_name, "w+") as f:
                    ...
                    print(
                        f"The content length of the remote file is: {content_length} bytes"
                    )
                    while True:
                    ...
                    if content_length and (int(file_size) != int(content_length)):
                        raise ValueError(
                            f"The file transfer was incomplete. Transferred only {file_size} bytes. Expected were {content_length} bytes"
                        )
                if checksum_before:
                    stdin, stdout, stderr = ssh_client.exec_command(
                        f"sha256sum {remote_name}"
                    )
                    checksum_after = stdout.read()
                    checksum_after = checksum_after.split()[0]

                    print(
                        f"The calculated checksum after the transfer is: {checksum_after}"
                    )
                    if checksum_before.strip() != checksum_after.strip():
                        raise ValueError(
                            f"The file transfer was incomplete. Checksum mismatch ({checksum_after} != {checksum_before}"
                        )
